...
public class DBActionListCreationTest extends AbstractTest {
  ...
  public void cleanup() throws SQLException{
    ...
    DBTable<Marque> marquesTable = database.getDBTable(new Marque());
		...
		assertThat(updateList.size(), is(1));

		final DBAction firstAction = updateList.get(0);
		final DBActionList revertList = firstAction.getRevertDBActionList();
		assertThat(firstAction, instanceOf(DBUpdateSimpleTypes.class));
		assertThat(revertList.size(), is(1));
		assertThat(revertList.get(0), instanceOf(DBUpdateToPreviousValues.class));
		List<String> revertStrings = revertList.get(0).getSQLStatements(database);
		assertThat(revertStrings.size(), is(1));
    ...
    DBTable<Marque> marquesTable = database.getDBTable(new Marque());
		...
		assertThat(updates.get(0), instanceOf(DBUpdateSimpleTypes.class));
		assertThat(updates.get(1), instanceOf(DBUpdateSimpleTypes.class));

		DBActionList reverts = updates.getRevertActionList();
		assertThat(reverts.size(), is(2));
		...
		assertThat(toyota.uidMarque.getValue(), is(toyotaUID));
		assertThat(ford.updateCount.getValue(), is(fordOriginalUpdateCount));
	}
	...
	public void insertAndRevertWithAutoIncrementTest() throws SQLException {
		...
		assertThat(foundHulme.size(), is(1));
	...
	public void insertLargeObjectAndRevertTest() throws SQLException, FileNotFoundException, IOException {
		...
		assertThat(foundLogo.size(), is(1));
		DBActionList revertActionList = insertActions.getRevertActionList();
		revertActionList.execute(database);
		foundLogo = database.get(example);
		assertThat(foundLogo.size(), is(0));
	...
	public void deleteAndRevertTest() throws SQLException {
		...
		assertThat(foundToyota.size(), is(1));

		DBActionList deleteActions = database.delete(example);
		assertThat(deleteActions.size(), is(1));
		assertThat(deleteActions.get(0), instanceOf(DBDeleteByExample.class));

		foundToyota = database.get(example);
		assertThat(foundToyota.size(), is(0));

		DBActionList revertActionList = deleteActions.getRevertActionList();
		assertThat(revertActionList.size(), is(1));
		assertThat(revertActionList.get(0), instanceOf(DBInsert.class));
	...
	public void deleteUsingAllColumnsAndRevertTest() throws SQLException {
		...
		assertThat(deleteActions.size(), is(1));
		assertThat(deleteActions.get(0), instanceOf(DBDeleteUsingAllColumns.class));
    ...
    Marque ford = marquesTable.getOnlyRowByExample(example);
		...
		assertThat(toyota.uidMarque.getValue().intValue(), is(toyotaUID));

		example = new Marque();
		example.name.permittedValuesIgnoreCase("ford");
		ford = database.getDBTable(example).getOnlyRowByExample(example);
		assertThat(ford.updateCount.getValue().intValue(), is(fordOriginalUpdateCount));

		example = new Marque();
		example.name.permittedValuesIgnoreCase("tvr");
		DBTable<Marque> rowsByExample = database.getDBTable(example);
		assertThat(rowsByExample.getAllRows().size(), is(0));

		example = new Marque();
		example.name.permittedValuesIgnoreCase("honda");
		rowsByExample = database.getDBTable(example);
		assertThat(rowsByExample.getAllRows().size(), is(1));
	}
	...
	public void revertListIsReversedTest() throws SQLException, UnexpectedNumberOfRowsException, FileNotFoundException, IOException {
		...
		assertThat(dataChanges.get(0), instanceOf(DBInsert.class));
		assertThat(dataChanges.get(1), instanceOf(DBUpdateSimpleTypes.class));
		assertThat(dataChanges.get(2), instanceOf(DBDeleteByPrimaryKey.class));
		assertThat(dataChanges.get(3), instanceOf(DBInsert.class));
		assertThat(dataChanges.get(4), instanceOf(DBUpdateLargeObjects.class));

		final DBActionList revertActionList = dataChanges.getRevertActionList();

		assertThat(revertActionList.get(0), instanceOf(DBDelete.class));
		assertThat(revertActionList.get(1), instanceOf(DBInsert.class));
		assertThat(revertActionList.get(2), instanceOf(DBUpdate.class));
		assertThat(revertActionList.get(3), instanceOf(DBDelete.class));
