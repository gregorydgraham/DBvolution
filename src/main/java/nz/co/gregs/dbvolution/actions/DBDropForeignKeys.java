 ...
 */
public class DBDropForeignKeys extends DBAction {

	private static final long serialVersionUID = 1L;
	private static final Log LOG = LogFactory.getLog(DBDropForeignKeys.class);

	public <R extends DBRow> DBDropForeignKeys(DBRow table) {
		super(table, QueryIntention.DROP_FOREIGN_KEYS);
	}

	@Override
	protected DBActionList getRevertDBActionList() {
		DBActionList reverts = new DBActionList();
		reverts.add(new DBCreateForeignKeys(getRow()));
		return reverts;
	}
		...
		if (definition.supportsAlterTableAddConstraint()) {
			...
			for (var field : fields) {
				if (field.isColumn() && !field.getQueryableDatatype().hasColumnExpression()) {
					final String fkSQL = definition.getAlterTableDropForeignKeyStatement(table, field);
	...
	protected DBActionList prepareActionList(DBDatabase db) throws AccidentalBlankQueryException, SQLException, UnableToInstantiateDBRowSubclassException {
		final DBDropForeignKeys newAction = new DBDropForeignKeys(getRow());
